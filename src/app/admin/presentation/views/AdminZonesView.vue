...
async function handleSubmit() {
  const err = validate()
  if (err) {
    feedback.value = { ok: false, msg: err }
    return
  }
  ...
  feedback.value = ok
    ...
    : {
        ok: false,
        msg: 'Ocurrió un error, verifica los datos e intenta de nuevo',
      }
...
function classColor(c: string) {
  return (
    { LIBRE: '#38a169', MODERADO: '#f2894a', OCUPADO: '#e53e3e' }[c] ?? '#888'
  )
}
          ...
          <tr v-for="z in filtered" :key="z.id">
                ...
                <div class="occ-bar">
                  <div
                    ...
                    :style="{
                      ...
                      background: classColor(z.classification),
                    }"
                  />
                </div>
                <span
                  class="occ-pct"
                  :style="{ color: classColor(z.classification) }"
                  >{{ Math.round(z.occupancyPercentage) }}%</span
                >
              </div>
            </td>
            <td>
              <span
                class="badge"
                :style="{
                  background: classColor(z.classification) + '20',
                  color: classColor(z.classification),
                }"
              >
                {{ z.classification }}
              </span>
            ...
            <td>
              <div class="actions">
                ...
                <button class="action-btn delete" @click="confirmId = z.id">
                  Eliminar
                </button>
              </div>
            </td>
      ...
      >
        <div class="confirm-box">
          ...
          <div class="confirm-actions">
            <button class="btn-ghost" @click="confirmId = null">
              Cancelar
            </button>
            <button class="btn-danger" @click="handleDelete(confirmId!)">
              Eliminar
            </button>
          </div>
        </div>
      ...
      <div v-if="showModal" class="overlay" @click.self="closeModal">
        <div class="modal">
          <h2 class="modal-title">
            {{ editTarget ? 'Editar zona' : 'Nueva zona' }}
          </h2>
          ...
          <div class="form-group">
            ...
            <input
              v-model="form.name"
              type="text"
              placeholder="Ej: Zona Norte, Zona Centro, Estacionamiento Sur"
            />
            <p class="field-hint">
              Nombre interno con el que identificarás esta zona.
            </p>
          </div>
          ...
          <div class="form-group addr-wrap">
            <label
              >Buscar dirección
              <span class="optional">(autocompletado)</span></label
            >
            <div class="addr-box">
              <svg
                width="14"
    ...
    <Transition name="modal">
      <div v-if="showSpaces" class="overlay" @click.self="closeSpaces">
        <div class="modal spaces-modal">
          ...
          <p v-if="spacesStore.error" class="err-msg">
            {{ spacesStore.error }}
          </p>

          <div v-if="spacesStore.loading" class="spaces-state">
            Cargando espacios...
          </div>
          <div v-else-if="spacesStore.spaces.length === 0" class="spaces-state">
          ...
          <div class="spaces-footer">
            <span class="spaces-count"
              >{{ spacesStore.spaces.length }} espacio{{
                spacesStore.spaces.length !== 1 ? 's' : ''
              }}</span
            >
